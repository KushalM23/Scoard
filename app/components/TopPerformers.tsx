...
const TopPerformers: React.FC<TopPerformersProps> = ({
  ...
  const getTopPerformers = (players: Player[]) => {
    return [...players].sort((a, b) => b.points - a.points).slice(0, 2);
  };
  ...
  const renderPerformerCard = (
    ...
    <div
      key={player.personId}
      className="bg-white/5 rounded-lg p-3 flex items-center gap-3 border border-white/5"
    >
  ...
  return (
    <div className="glass-card p-4 mb-4 max-w-2xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Home Team */}
        <div>
          <h4 className="text-sm font-display text-primary mb-2 uppercase tracking-wider text-center">
            {homeTeamName}
          </h4>
        ...
        {/* Away Team */}
        <div>
          <h4 className="text-sm font-display text-secondary mb-2 uppercase tracking-wider text-center">
            {awayTeamName}
          </h4>
          <div className="flex flex-col gap-2">
