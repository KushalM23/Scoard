...
export default function TransactionsClient() {
  ...
  // isOffseason check removed — page title is always contextual by tab

  const pageTitle = useMemo(() => {
    if (activeTab === "draft") return "NBA Draft History";
    if (activeTab === "trades") return "Trades";
    return "Transactions Board";
  }, [activeTab]);
  ...
  const getBadgeStyle = (type: string) => {
    ...
    if (norm === "signing" || norm === "contractconverted") {
      return "bg-status-signing/20 text-status-signing border-status-signing/40";
    }
    if (norm === "trade") {
      return "bg-status-trade/20 text-status-trade border-status-trade/40";
    }
    if (norm === "waive") {
      return "bg-status-waive/20 text-status-waive border-status-waive/40";
    }
    return "bg-status-other/20 text-status-other border-status-other/40";
  };
            ...
            {filteredDraftPicks.length === 0 ? (
              ...
              <>
                ...
                {draftRounds.round1.length > 0 && (
                  <section className="rounded-2xl bg-surface-card/80 p-5 sm:p-6">
                    <h2 className="text-xl sm:text-2xl font-display font-bold text-text mb-5 border-b border-surface-borderLight pb-2 uppercase">
                      Round 1 Picks
                    </h2>
                    <div className="overflow-x-auto">
                      <table className="w-full text-left text-sm sm:text-base border-collapse">
                        <thead>
                          <tr className="border-b border-surface-borderLight text-text/40 text-[10px] sm:text-xs font-bold tracking-wider uppercase">
                        ...
                        <tbody className="divide-y divide-surface-borderLight/40">
                ...
                {filteredTransactions.map((tx, idx) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className="rounded-2xl border border-surface-border bg-surface-card/50 p-4 sm:p-5 flex flex-col sm:flex-row gap-4 items-start sm:items-center relative hover:border-surface-elevated transition-colors"
                    >
                      ...
                      <div className="flex gap-2 sm:flex-col shrink-0 w-full sm:w-auto sm:border-l border-surface-border/60 sm:pl-4 pt-3 sm:pt-0 border-t sm:border-t-0 mt-2 sm:mt-0">
                        {hasPlayer && (
                          <PlayerLink
                            playerId={tx.PLAYER_ID}
                            className="flex-1 sm:flex-none text-center sm:text-left px-3 py-1 rounded-lg bg-white/5 hover:bg-status-trade/20 hover:text-status-trade transition-all text-xs font-display tracking-wide flex items-center justify-center sm:justify-start gap-1 w-full uppercase"
                          >
                        ...
                        {hasTeam && (
                          <TeamLink
                            teamId={tx.TEAM_ID}
                            className="flex-1 sm:flex-none text-center sm:text-left px-3 py-1 rounded-lg bg-white/5 hover:bg-status-signing/20 hover:text-status-signing transition-all text-xs font-display tracking-wide flex items-center justify-center sm:justify-start gap-1 w-full uppercase"
                          >
                            <span>Team Page</span>
                            <ChevronRight className="w-3.5 h-3.5" />
                          </TeamLink>
