...
const Scoreboard: React.FC<ScoreboardProps> = ({ gameData }) => {
  const { homeTeam, awayTeam, period, gameStatusText, gameStatus, gameEt } =
    gameData;
      ...
      >
        ...
        <div className="flex flex-col items-center relative">
          ...
          <TeamLink
            ...
            sourceComponent="scoreboard"
            className={`text-xl md:text-2xl font-display hover:text-accent transition-colors ${isHomeLeading ? "text-primary" : "text-text"}`}
          >
        ...
        <div className="flex flex-col items-center min-w-[120px]">
          ...
          <div className="flex flex-col items-center mb-2">
            {isScheduled ? (
              <div className="flex flex-col items-center gap-2">
                <span className="text-[10px] font-bold text-text/50 tracking-widest uppercase">
                  TIPOFF (IST)
                </span>
          ...
          {!isScheduled && (
            <div className="flex items-center gap-4">
              <span
                className={`text-3xl md:text-4xl font-mono font-medium tracking-tighter ${isHomeLeading ? "text-primary" : "text-text"}`}
              >
                {homeTeam.score}
              </span>
              <span className="text-text/10 text-2xl font-light">-</span>
              <span
                className={`text-3xl md:text-4xl font-mono font-medium tracking-tighter ${isAwayLeading ? "text-secondary" : "text-text"}`}
              >
                {awayTeam.score}
              </span>
            </div>
        ...
        <div className="flex flex-col items-center relative">
          ...
          <TeamLink
            ...
            sourceComponent="scoreboard"
            className={`text-xl md:text-2xl font-display hover:text-accent transition-colors ${isAwayLeading ? "text-secondary" : "text-text"}`}
          >
      ...
      {!isScheduled && (
        <div className="hidden sm:block overflow-x-auto mt-4 border-t border-text/10 pt-2">
          <table className="w-full text-center border-collapse">
            ...
            <tbody>
              <tr>
                <td className="p-1 text-left font-bold text-text text-sm">
                  {homeTeam.teamTricode}
                </td>
                {homeTeam.periods.map((score, i) => (
                  <td
                    key={i}
                    className={`p-1 text-sm ${score > awayTeam.periods[i] ? "text-primary font-bold" : "text-text/80"}`}
                  >
                    {score}
                  </td>
                ))}
                <td className="p-1 font-bold text-sm text-text">
                  {homeTeam.score}
                </td>
              </tr>
              <tr>
                ...
                {awayTeam.periods.map((score, i) => (
                  <td
                    key={i}
                    className={`p-1 text-sm ${score > homeTeam.periods[i] ? "text-primary font-bold" : "text-text/80"}`}
                  >
                    {score}
                  </td>
                ))}
